...
Modified version of code from: https://github.com/xternalz/WideResNet-pytorch
'''
import torch
...
class BasicBlock(nn.Module):
    def __init__(self, in_planes, out_planes, stride, dropRate=0.0, scales=[1]):
        ...
        self.conv1 = Kanazawa_SIConv2d(in_planes, out_planes, kernel_size=3,
                                       stride=stride, padding=1, bias=False, scales=scales)
        self.bn2 = nn.BatchNorm2d(out_planes)
        self.relu2 = nn.ReLU(inplace=True)
        self.conv2 = Kanazawa_SIConv2d(out_planes, out_planes, kernel_size=3, stride=1,
                                       padding=1, bias=False, scales=scales)
...
class NetworkBlock(nn.Module):
    ...
    def _make_layer(self, block, in_planes, out_planes, nb_layers, stride, dropRate, scales):
        layers = []
...
class WideResNet(nn.Module):
    def __init__(self, depth, num_classes, widen_factor=1, dropRate=0.0, scales=[1]):
        ...
        self.conv1 = Kanazawa_SIConv2d(3, nChannels[0], kernel_size=3, stride=1,
                                       padding=1, bias=False, scales=scales)
        # 1st block
...
def wrn_16_8_kanazawa(num_classes, **kwargs):
    scales = [0.5, 1.0, 2.0]
