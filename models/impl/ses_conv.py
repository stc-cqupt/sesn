...
class SESConv_H_H(nn.Module):
    ...
    def naive(self, x):
        ...
        B, C, S, H, W = x.shape
        assert S == self.num_scales
        if self.scale_size != 1:
            x = F.pad(x, [0, 0, 0, 0, 0, self.scale_size - 1])

        output = []
        num_output_scale = x.size(2) + 1 - self.scale_size
        print('output S {}'.format(num_output_scale))
        for idx_s in range(num_output_scale):
            this_output = 0
            for kernel_scale_idx in range(self.scale_size):
                if kernel_scale_idx < len(self.scales):
                    s1 = self.scales[0]
                    s2 = self.scales[kernel_scale_idx]
                    mult = s2 / s1
                    print(mult)
                else:
                    mult = 1.0
                K = kernel[:, :, kernel_scale_idx, idx_s]
                T = x[:, :, idx_s + kernel_scale_idx]
                this_output += mult * F.conv2d(T, K, padding=self.padding)
