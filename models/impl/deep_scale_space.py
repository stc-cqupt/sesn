...
class BesselConv2d(nn.Module):
    ...
    def _get_blur(self):
        ...
        for i, (scale, width) in enumerate(zip(self.scales, self.widths)):
            ...
            kernelx = nn.Parameter(kernelx, requires_grad=False)
            kernely = nn.Parameter(kernely, requires_grad=False)

            self.register_parameter('kernelx_{}'.format(i), kernelx)
            self.register_parameter('kernely_{}'.format(i), kernely)
